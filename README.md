...
离线每一帧的识别逻辑（与在线 `trace_calculator` 同源），具体滑条用法、参数含义详见同级目录下 `调试参数/运行说明.txt`。调好后按 **k** 保存，生成 `offline_trace_params_*.txt`。
...
我调参数的顺序（A→B→C→D）调多了就发现，按这个顺序来最不容易乱。
...
```bash
bash trace_energy_ws/start_pidtest_visual.sh
```
...
```bash
ros2 launch simple_track simple_track_full.launch.py
```
